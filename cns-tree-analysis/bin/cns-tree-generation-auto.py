...
#note: load the python3/3.5.0, samtools/1.9, and python3/3.5.0_packages/pandas/0.18.0 modules before running this
#make sure geneID versions match the versions in the protein database (Maize NAM v5)
...
combinedCsvPath = cnsPath + '/combinedCNS.csv'
if (os.path.exists(combinedCsvPath) == False): #combinedCNS.csv: All of the maize CNSs from conservatory/CNS/ concatenated together with cat
    print("Missing 'combinedCNS.csv'")
    exit()
bamPath = cnsPath + '/Poaceae.bam'
if (os.path.exists(bamPath) == False): #the final conserved ortholog alignment file from conservatory/alignments/
    print("Missing 'Poaceae.bam'")
    exit()
if (os.path.exists(cnsPath + '/Poaceae.bam.bai') == False): #the index file for the above from conservatory/alignments/
    print("Missing 'Poaceae.bam.bai'")
    exit()
allOrthologsPath = cnsPath + '/all.orthologs.csv'
...
#make sure txt file has no invisible whitespace chars
dos2unixCmd = str("dos2unix " + myGeneID + "_TreeGenes.txt")
subprocess.call(dos2unixCmd, shell=True)
...
def extractAlignments(gene):
    cnsIDs = []
    total_count = 0
    with open(combinedCsvPath, "r") as fp:
        for line in lines_that_contain(gene, fp):
            ...
            samtoolsCmd = str("samtools view " + bamPath + " " + lineArray[5] + ":" + lineArray[6] + "-" + lineArray[7] + " | awk '{print $1}' | awk -F: '{print $3}' > " + cnsID + ".txt")
...
def dataframeMerge(geneListName, df):
    with open(str(geneListName + ".txt")) as data:
        newGenes = data.read().split('\n')
    newGenes = filter(None, newGenes) #make sure there are no empty strings
    df[str(geneListName)]=df['tree_genes'].str.contains('|'.join(newGenes))
    updatedDf = df
    return updatedDf
...
def falsePosCheck(gene):
    subprocess.call("grep " + gene + " " + allOrthologsPath + " | awk -F, '{print $1}' > " + gene + ".orthologs.txt", shell=True)
